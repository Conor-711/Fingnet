...
import { MessageCircle, User, Brain, Target, Sparkles, Clock, Settings, CreditCard, Send, Inbox, Users, Plus, Trash2, LogOut, ChevronUp, ChevronDown, Loader2, Check, ArrowUp } from 'lucide-react';
...
const Main = () => {
  ...
  const renderAITwinPage = () => {
    ...
    const getDailyModelingHistory = () => {
      if (!user) return [];
      const history = JSON.parse(localStorage.getItem(`dailyModeling_${user.id}_history`) || '[]');
      return history.slice(-10).reverse(); // 最近10条，倒序
    };
    ...
    <div className="max-w-7xl mx-auto">
      {/* Page Title */}
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-gray-900 mb-2">You & {aiTwinProfile?.name || 'Your AI Twin'}</h2>
        <p className="text-gray-600">Explore your profile and {aiTwinProfile?.name || 'Your AI Twin'}'s learned insights</p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-6">
            ...
            <CardContent className="p-6">
              ...
              <div className="flex items-center space-x-4 mb-6 pb-6 border-b border-gray-200">
                <Avatar className="w-20 h-20 ring-4 ring-white shadow-lg">
                  <AvatarImage src={aiTwinProfile.userAvatar || "/avatars/middle.png"} alt={aiTwinProfile.userNickname || "You"} />
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    <p className="text-sm text-gray-600">
                      <span className="font-medium">{aiTwinProfile.profile?.gender || 'N/A'}</span> • <span className="font-medium">{aiTwinProfile.profile?.age || 'N/A'}</span>
                    </p>
          ...
          <Card className="shadow-lg border-0 bg-gradient-to-br from-emerald-50 to-teal-50">
            ...
            <CardContent className="p-6">
              ...
              <div className="flex items-center space-x-4 mb-6 pb-6 border-b border-gray-200">
                <Avatar className="w-20 h-20 ring-4 ring-white shadow-lg">
                  <AvatarImage src={aiTwinProfile.avatar || "/avatars/middle.png"} alt={aiTwinProfile.name || "AI Twin"} />
              ...
              <div className="space-y-6">
                {/* Learned from User (Daily Modeling) */}
                <div>
                  <h4 className="font-semibold text-gray-900 mb-3 flex items-center">
                  ...
                  {dailyHistory.length > 0 ? (
                    <div className="space-y-3">
                      {dailyHistory.slice(0, expandedFields.learnedFromUser ? dailyHistory.length : 3).map((entry, index) => (
                ...
                {/* Memory (from Group Chat Summaries) */}
                <div>
                  <h4 className="font-semibold text-gray-900 mb-3 flex items-center">
      ...
      {user && (
        <div className="fixed bottom-8 right-8 z-50 flex flex-col gap-2">
          ...
          {showScrollToTop && (
            <Button
              onClick={scrollToTop}
              className="bg-white hover:bg-gray-50 text-gray-700 border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-200"
              size="sm"
            >
